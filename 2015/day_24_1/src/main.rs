...
#[allow(dead_code)]
fn count_mask(mask: u32) -> u32 {
    mask.count_ones()
}

fn sum_mask(values: &[u32], mask: u32) -> u32 {
    masked_values(values, mask).sum()
}

fn quantum_entanglement_mask(values: &[u32], mask: u32) -> u64 {
...
fn valid_candidates_for_sum(values: &[u32], target: u32) -> impl Iterator<Item = u32> + '_ {

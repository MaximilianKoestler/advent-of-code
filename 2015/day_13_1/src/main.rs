...
fn build_name_map(entries: &[Information]) -> HashMap<Name, usize> {
    entries
        .iter()
...
fn compute_optimal_seating(matrix: &[Vec<Option<i32>>]) -> (Vec<usize>, i32) {
    (0..matrix.len())
...
fn main() {
    ...
    let (_, happiness) = compute_optimal_seating(&matrix);
